...
let erase_fp (fic, backend) terminate =
...
let main out err terminate version verbose_ where config summary_ timing search_dir keep_going_
  ...
  let () =
    final version (fun () -> show_version out terminate)
    *** set verbose_ verbose
    *** final where (fun () -> show_where out terminate)
    ...
    *** setf erasefp (fun fic -> erase_fp fic terminate)
    *** setf printfp (fun fic -> print_fp fic terminate)
    *** setf usefp use_fp
    *** set fpp fp_deb
  ...
  if config then terminate 0;
  if mods = [] then begin
    Format.fprintf err "Need at least one module file.\n%!";
    terminate 2;
  end;
  ...
  if !Params.toolbox then begin
    ...
    Array.iter (fun s -> Format.fprintf out " %s" (quote_if_needed s)) Sys.argv;
    Format.fprintf out "\n\n%!"
  end;
...
let term out err terminate =
  Term.(const (main out err terminate) $ version_t $ verbose_t $ where_t $ config_t $ summary_t $ timing_t
...
let cmd out err terminate =
  let info = Cmd.info "tlapm" in
  Cmd.v info (term out err terminate)
...
let init ?(out=Format.std_formatter) ?(err=Format.err_formatter) ?(terminate=exit) (executable_name : string) (args : string array) =
  let argv = args |> Array.to_list |> preprocess |> Array.of_list in
  match Cmd.eval_value' ~help:out ~err ~argv (cmd out err terminate) with
  | `Ok x -> x
  | `Exit r -> terminate r; []
